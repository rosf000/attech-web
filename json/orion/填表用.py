...
# 1. 要讀取的 JSON 檔案清單
FILE_LIST = [
    "coating.json","Inkandimpactprinting.json"
]
# 2. 【核心需求】要統一自動補上的 Brand Value（可依需求修改此字串）
DEFAULT_BRAND_CODE = "orion"
...
def transform_product(item):
    ...
    # ============================================================
    # 【自動補上 Brand Code】
    # 優先使用原資料內的 brand_code；若為空或不存在，自動填入指定的 DEFAULT_BRAND_CODE
    # ============================================================
    ...
    res["composition_zh"] = clean_value(item.get("composition_zh"))
    res["composition_en"] = clean_value(item.get("composition_en"))
    res["chemical_component"] = clean_value(item.get("composition_en"))
